...
func SeedEmpresaTeste(db *pgxpool.Pool) error {
	ctx := context.Background()

	// 1. Verifica se a empresa TESTE já existe
	var existe bool
	err := db.QueryRow(ctx, "SELECT EXISTS(SELECT 1 FROM empresas WHERE id = 2)").Scan(&existe)
	if err != nil {
		return err
	}

	if existe {
		log.Println("Empresa de teste já configurada. Pulando seed.")
		return nil
	}

	log.Println("Configurando Empresa teste pela primeira vez...")
	...
	// 3. Pega a senha do arquivo .env (que fica seguro no servidor/Docker)
	senhaPadrao := os.Getenv("USER_TESTE_PASSWORD")
	if senhaPadrao == "" {
		senhaPadrao = "Mudar@123" // Fallback de segurança
	}
	...
	// 5. Insere o usuário
	emailAdmin := os.Getenv("USER_EMAIL")
	if emailAdmin == "" {
    emailAdmin = "admin@radaptech.com.br" // Email padrão de segurança
    }
	_, err = db.Exec(ctx, `
        INSERT INTO usuarios (tenant_id, nome, email, senha_hash, ativo, role)
        VALUES (2, 'davi', $1, $2, true, 'admin')
    `, emailAdmin, string(hash))
	...
	log.Println("Empresa teste e user teste criados com sucesso!")
	return nil
}
